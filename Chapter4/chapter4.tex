...
% TODO Turn to the more niche use of LIWC and describe in detail.
% TODO Mention that most commonly SVMs and LRs are used.
All features must be weighted, either through raw counts or their relative frequency. One such frequently used weighting scheme is Term Frequency - Inverse Document Frequency (TF-IDF) which weights features by their relative frequency in the corpus \citep{Fortuna:2018}, assigning higher weight to the features that are rare corpus-wide and lower weights to those that common.
As such, TF-IDF can be a useful measure to address the dominance of high-frequency tokens. At the same time, TF-IDF also increases the capacity for models to overfit to the corpus and generalise poorly, as tokens that are unique to a corpus may not exist in other data or even be common to other data. The use of n-grams as features provides a similar double-edged benefit, where models learn sequences of words, in abuse detection the most commmon n-grams are unigrams, bi-grams, and trigrams. Such word-sequences can be helpful for models in uncovering patterns of language use in the corpus but are also sensitive to the vocabulary changes that occur across datasets. For instance \citet{Waseem-Hovy:2016} train a Logistic Regression classifier and identify that character n-grams of innocuous words such as `islam' and `Muslim' rank as some of the most predictive features due to the disproportionate occurrences of such terms in the hateful classes.


For the tasks of identifying online abuse, LIWC has previously been proposed as a f
For instance, to perform the task of predicting abuse in the context of Massive Open Online Courses (MOOCs) \citet{Joksimovic:2019}

; n-grams that consider sequential tuples of tokens; and POS tags \cite{Fortuna:2018}. LIWC has also been examined in the context of abusive detection, \citet{Nina-Alcocer:2019} use LIWC to compute percentages of emotions that are present in abusive documents. They also incorporate ``sexual, anxiety feeling, anger, and so on'' into their feature representation, however they do not clarify what such features may be or how they are incorporated.

In many of the studies that use features crafted on the basis of direct hypotheses, performance is improved as a result, albeit only marginally in some cases. For instance \citet{Waseem-Hovy:2016} show a small improvement by including the predicted gender of the author as a feature; \citet{Davidson:2017} show that POS tags can aid in the prediction. A number of studies \citep{Waseem-Hovy:2016,Nobata:2016,...} show the benefit of using character-level n-grams over word-level n-grams \citep{Fortuna:2018}, \citet{Waseem:2017} suggest that improved performance may be a result of a more dense feature matrix provided as training data for the model. Most relevantly, \citet{Nina-Alcocer:2019} show that using information obtained through the LIWC dictionary can aid in classification of misogyny, in particular when used in combination with other feature representations such as word-level n-grams.
...
SVM \cite{Joksimovic:2019}
Rule-based \cite{Gorrel}

\subsubsection{Neural Networks}

Neural network methods have frequently been applied to the problem of abusive language detection and hate speech detection, frequently posting ever-evolving State-of-the-Art models and classification performances \citep[e.g.]{Badjatiya:2017,Zimmerman:2018,Isaksen:2020}. In the effort to obtain State-of-the-Art classification performance, a number of modelling approaches have been proposed in structuring of the task as well as the specific implementation choices made. For instance, where \citet{Badjatiya:2017} and \citet{Isaksen:2020} keep the proposed task formulation of hate speech detection as a three-way multi-class classification problem provided by \citet{Waseem-Hovy:2016}, \citet{Park:2017} propose instead to split the classification problem into two-tiered problem, where in the first tier, a classifier is trained to predict whether a document contains abuse and the second tier operates only on the positive instances, predicting the same three-way classification task as \citet{Isaksen:2020,Waseem-Hovy:2016}.
