...
Focusing on the non-abusive auxiliary tasks, only the \textit{Sarcasm} auxiliary task is selected for further experimentation given the results on the validation data.
...
However, the \textit{Argument Basis} auxiliary tasks outperforms the \textit{Sarcasm} on the validation data in terms of accuracy and recall, suggesting that it may be beneficial to look beyond the F1-score for ranking and selecting auxiliary tasks.
...
For the machine learning models optimise on the \textit{Toxicity} dataset, none of the models outperform the linear SVM in terms of F1-score.
All configurations outperform the MLP in terms of F1-score, precision and accuracy.
However, no configuration of the auxiliary tasks outperform the MLP baseline or the ensemble baseline, the highest performing baseline.
The best performing model in terms F1 scores are models that have at least one auxiliary task that is an abusive language task.
...
\zw{Argument basis generally not helpful}

\section{Conclusions}

\zw{Benefit from using non-abusive tasks in conjunction with abusive tasks}
\zw{Benefit from using abusive tasks that have the same dataset source}
