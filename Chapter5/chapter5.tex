...
One of the key issues in machine learning for content moderation is that such systems in deployed settings (see \cref{chap:filter}) as well as in research (see \cref{chap:intro} and \cref{chap:nlp}) over-fit to individual tokens that are over-represented in the positive and negative classes respectively.
Moreover, efforts in NLP have identified that content moderation systems are likely to over-fit to identity markers such as the mentions of gender and race \citep{Dixon:2018}.
...
\ZTdelete{Although some such approaches have}\ZTedit{Some work has} addressed this problem by replacing certain words and phrases to balance identity distributions \cite{Dixon:2018,Park_2018}.
...
An additional benefit of such vocabulary reduction is a decrease in model size and optimisation time required for complex models such as neural networks, resulting in models that have a smaller environmental impact \citep{Strubell:2019}.
...
Through this use of LIWC \cite{Pennebaker:2015,Pennebaker:2001}, I pre-process documents from large vocabularies, that are riddled with obfuscations, intentional misspellings, and unintentionally misspelled words into a smaller vocabulary set representing instead psycholinguistic properties of words.
...
\ZTedit{Through the use of simple deep neural networks and `shallow' linear models, I show that it is possible to achieve comparable performances within datasets and, in some cases, improvements on out-of-domain datasets, in spite of up to $98\%$ reductions in model parameters and vocabulary size.}
\ZTdelete{Through the use of simple deep neural networks and `shallow' linear models, I show that by reducing the vocabulary sizes by up to $98.0\%$ and the number of model parameters by up to $98\%$, while increasing the depth of the information in the remaining vocabulary, it is possible to achieve comparable performances within datasets and, in some cases, improvements on out-of-domain datasets.}
...
Finally, as datasets for hate speech detection frequently contain biases along racialised and dialectal lines \citep{Waseem:2018,Davidson:2019}, the use of LIWC can serve as small, but conflicted aid in avoiding such biases as dialectal spellings of words are \ZTdelete{likely to not}\ZTedit{unlikely to} appear in the dictionary, thus being relegated to unknown tokens (see \cref{tab:LIWC_tok} for synthetic examples of LIWC representations).
...
In this thesis, I use LIWC to provide the word categories that each word invokes, and rather than compute the overall word classes exhibited to provide an analysis, I use the LIWC categories of each word in a document as an alternative \ZTedit{document} representation\ZTdelete{ of the document from which percentages of word categories invoked can be recovered}.
...
\ZTedit{LIWC, as a psychometric tool is embedded within this troubling history and development.
...
\ZTedit{In the work described below, there is a heavy reliance on LIWC, however it is not used to predict the emotional and mental states of the speakers of given texts.
...
\ZTdelete{The primary reasons for}\ZTedit{There are two primary reasons for} using manually crafted features\ZTdelete{ is two-fold}: First, \ZTdelete{by using}\ZTedit{using} manually crafted features \ZTdelete{it is necessary to have}\ZTedit{requires having} some understanding of the data at hand and some intuition about which features may distinguish the classes in the data from one another.
...
\ZTdelete{In a}\ZTedit{Through a} consideration of a handful of systems that employ some of the most frequently used features for the development of \ZTedit{machinic abusive language detection systems}\ZTdelete{automated systems for detecting various forms of online abuse}, distinct modelling choices, features and rationales for their use become \ZTdelete{prominent}\ZTedit{apparent}.
...
First, the most common feature used, and rarely used on its own, is a Bag-of-Words (BoW) \citep{Fortuna:2018,Davidson:2017}, where each token in a document is treated as independent from \ZTdelete{the remainder of}\ZTedit{all other tokens in} the document.
The use of this feature frequently relies on \ZTdelete{the use of}\ZTedit{using} stop-word lists to remove tokens that are bound to occur frequently across a majority of documents, \ZTdelete{such as}\ZTedit{e.g.} determiners, to \ZTdelete{avoid}\ZTedit{prevent} models from learning spurious correlations with such \ZTdelete{words}\ZTedit{tokens}\ZTdelete{ and an individual class due to fluctuations in the data}.
...
Several works have complicated this notion \citep[e.g.]{Waseem:2018,Davidson:2019}, arguing that tokens \ZTedit{considered} in isolation do not provide the necessary context to determine whether a text is abusive.
...
Here, by considering groups of sequential token occurrences independently from \ZTdelete{one another}\ZTedit{token sequences}, a step is taken away from the independence of individual tokens, instead to the independence of short sequences of tokens.
...
Another frequently used feature is sentiment \ZTedit{scores obtained using sentiment} analysis \citep{Fortuna:2018} \ZTdelete{with}\ZTedit{which have} the underlying assumption that abuse and negative sentiment are correlated, and can thus aid in detecting some forms of abuse.
Similarly to BoW and n-grams, this is a feature that is most frequently used in combination with other features as sentiment \ZTedit{scores} alone \ZTdelete{is}\ZTedit{are} not presumed to be good predictors of abuse \citep{Fortuna:2018}.
...
This use of LIWC, similar to the motivations for its use in this chapter, relies on an assumption that \ZTdelete{the}\ZTedit{proxies of} mental states of the speaker and the interpretations of readers will relay information on the intention of the speaker to cause offence.
For instance, \citet{Nina:2018} compute the percentages of emotions that are expressed in abusive documents in efforts to identify correlations between impassioned \ZTedit{and emotive} speech \ZTdelete{and}\ZTedit{with} abuse, asserting an intuition that abusive speech is likely to occur in individual moments dominated by emotion rather than rationality.
...
The use of n-grams as features provides a similar double-edged benefit, where models optimise on sequences of words \ZTedit{that may be very differently distributed in each class}.
...
Many of the previously mentioned works use \ZTdelete{similar}\ZTedit{linear} machine learning models, with a particular dominance of Logistic Regression and Support Vector Machines (SVMs) (please see \cref{chap:nlp} for more detail).
...
Rather than using supervised classification techniques, \ZTedit{they argue that their}\ZTdelete{a} rule-based systems\ZTdelete{was developed} to detect abuse\ZTdelete{ that they argue} allows for a interpretable and easy to modify method\ZTdelete{ to address}\ZTedit{, allowing researchers to address} weaknesses of the approach without the need for additional large\ZTedit{-scale} quantities of data.\footnote{This detail on the rule-based nature of the classification systems was provided by Genevieve Gorrell in personal communications.}
...
\ZTedit{
  ...
  A large body of work is dedicated to the mathematical \ZTedit{and algorithmic} induction of what features to retain and which to omit \citep[e.g.]{Witschel_2006}.
  The object of the mathematical \ZTedit{and algorithmic} approaches is to identify which features provide the most \ZTedit{and least} information about the distinctions between each class\ZTdelete{ and which features provide the least information, and therefore can be omitted when optimising a machine learning model}.
  ...
  Another, more recent approach is to perform partial replacements in documents, only replacing some tokens.
...
\ZTedit{
  ...
  For instance, \citet{Pamungkas_2020} use HurtLex to create a feature vector for each of the fine-grained categories to be used for optimisation and classification, as a way to separate out profanities and hateful terms.
...
As all neural network-based methods that I examine receive only the text as input, the primary differences between \ZTedit{different proposed neural network}\ZTdelete{the} models is in their use and organisation of different types of layers and the loss function selected for the respective models.
...
Prior to passing \ZTedit{data} to an output layer, \ZTedit{a} dropout \ZTedit{layer} is applied to the output of the convolutional layers which is then passed to a dense layer.
...
The use of LSTMs, that rely on recurrence, breaks with the independence assumption of the manual feature-based models.
...
For the work in this chapter, the use of pre-optimised embeddings is not appropriate for some models.
...
Second, I don't use pre-optimised embeddings in the architectures for other \ZTedit{models as a means} to ensure that any comparisons with the LIWC-based models, \ZTedit{provide}\ZTdelete{are} a direct comparison of the influence of using LIWC as input tokens\ZTdelete{and}\ZTedit{while} avoiding potentially confounding factors.
...
This has downstream implications for the construction of the datasets and for the validity of the prediction task on the \ZTedit{out-of-domain}\ZTdelete{auxiliary} datasets.
...
The exception to this is the dataset proposed by \citet{Davidson:2017} where the largest class is the `offensive' class, which I combine with `hateful', further minimising the \ZTdelete{relative} size of the negative class.
...
\ZTedit{I choose this subset as it allows for testing}\ZTdelete{to test} how LIWC-based models perform when optimised on a a) small dataset and b) balanced data distribution.
The dataset is initially split into an optimisation and evaluation set, I \ZTedit{further} create a validation set by extracting a stratified sample from the optimisation data, retaining the class balance from the balanced subset.

\subsubsection{Offence}
The second dataset \ZTedit{that I use}\ZTdelete{used} to optimise and evaluate my models is the \textit{Offence} dataset collected from Twitter by \citet{Davidson:2017}.
...
In this dataset \ZTedit{I collapse} the  `racism' and `sexism' classes\ZTdelete{are collapsed} into a single positive class, `abuse'. 
\ZTedit{This class} consists of $5,355$ documents \ZTdelete{and}\ZTedit{with} the negative class occupying the remaining $11,559$ documents.
The primary function \ZTedit{that} this dataset serves in this chapter is to allow \ZTdelete{some}\ZTedit{for} insight into whether the LIWC-based models would function under a distinct annotation criteria that is motivated by academic work in Gender Studies and Critical Race Theory on marginalisation, rather than social media guidelines for acceptable behaviour.
...
This dataset is annotated following the annotation guidelines proposed by \citet{Waseem-Hovy:2016}, however it is annotated using intersectional feminist activists as \ZTdelete{crowd-workers}\ZTedit{annotators}.
...
In order to understand the impact of LIWC-based modelling, I design feature-based and neural network\ZTedit{-based machine learning} models.
I optimise a Logistic Regression \ZTedit{model} and a SVM model with a linear kernel for each type of input data (Word unigrams, BPE unigrams, and LIWC unigrams) to allow for feature-based analysis of \ZTdelete{what patterns are identified}\ZTedit{the identified patterns}.
...
For the linear baseline models, I tokenise and pre-process the data and perform a grid-search over \ZTdelete{the}\ZTedit{a} parameter space \ZTdelete{that}\ZTedit{in search of the parameter values that} optimises for the highest macro \textit{F1-score} performance.
...
I then reuse this best performing parameter settings and re-run the models with 5 different random seeds to ensure that the \ZTedit{models'} behaviour \ZTdelete{of the models}on the dataset is not the result of the \ZTdelete{consequences of the random seed propagated into a model's} initialisation of \ZTdelete{its} tensors \ZTedit{caused by the setting of the random seed values}.

\subsection{Pre-processing}
Prior to providing \ZTedit{data to} any model\ZTdelete{data}, it is necessary to pre-process the data to make it suitable for the experiment conducted.
In my experiments I examine \ZTedit{the influences that vocabulary manipulation has on model design.}\ZTdelete{how modifying the vocabulary that a model relies on might influence model design.}
To this effect, it is necessary to have some \ZTedit{shared and} distinct pre-processing steps for the datasets, depending on the experiment\ZTdelete{ while others are shared}.
...
For the LIWC experiments\ZTdelete{ on the other hand}, I take another step after the initial tokenisation \ZTedit{so that I can}\ZTdelete{to} compute the LIWC categories invoked by each word.
...
For the BPE-based models on the other hand, I pre-process documents \ZTdelete{by computing} using the 200-dimensional Byte-Pair Embeddings from the BPE python library \citep{Heinzerling:2018}.
Byte-Pair Encodings are well suited to handle the particularities of social media text, as it breaks unrecognised words into \ZTedit{known} subwords, thus minimising unknown tokens in the validation and evaluation sets.
...
In reviewing \cref{tab:vocab_sizes}, it is clear that computing LIWC representations result in smaller vocabularies \ZTedit{than the surface form counter-parts}.
\ZTedit{It's further apparent that the BPE representations does not substantially alter the vocabulary sizes, with the exception of the \textit{Hate Expert} and \textit{Hate Speech} datasets, where there's a small increase in the vocabulary sizes.}
\ZTdelete{while BPE representations of the documents results in similar sized vocabularies for all datasets with the exception of the BPE representation of the \textit{Toxicity} dataset where the size of the BPE vocabulary sees a small drop in tokens and the \textit{Hate Expert, Hate Speech}, and \textit{StormFront} dataset where the vocabulary size increase slightly.}
It is unsurprising that the vocabulary size would grow \ZTedit{when} using BPE, as subwords for all unrecognised tokens are computed using the Byte-Pair Encoding.
\ZTedit{That is, when there are words in the dataset that aren't recognised by the embedding, the words are split into their sub-parts thus increasing the overall size of the dataset.}
\ZTdelete{More surprising is the drop in BPE vocabulary size for the \textit{Toxicity} data.
This drop suggests that there are words that are unrecognised in their full form by the pre-optimised Byte-Pair Embeddings.}
...
Consequently \ZTedit{the vocabularies produced using the LIWC dictionary is}\ZTdelete{will be} subject to \ZTdelete{some} language drift in addition to domain shifts.
...
\begin{tabular}{lllll}
  ...
  \textit{Offence}     & $24\; (2.8\%)$   & $150\; (17.6\%)$ & $677\; (79.6\%)$ & $851$\\
  \textit{Toxicity}    & $131\; (12.8\%)$ & $5\; (0.5\%)$    & $886\; (86.9\%)$ & $1,022$\\
  \textit{Hate Expert} & $241\; (32.6\%)$ & $25\; (3.4\%)$   & $473\; (64\%)$   & $739$  \\
  \textit{Hate Speech} & $116\; (13.9\%)$ & $47\; (5.62\%)$  & $674\; (80.5\%)$ & $837$\\
  \textit{StormFront}  & $74\; (11.9\%)$  & $117\ (18.8\%)$  & $431\; (69.3\%)$ & $622$
\end{tabular}%
}
...
\begin{tabular}{lllll}
                       & Not Abuse Only      & Abuse Only         & Intersection        & Vocab size\\\hline
  \textit{Offence}     & $3,303\; (19.7\%)$  & $8,656\; (51.6\%)$ & $4,809\; (28.7\%)$  & $16,768$\\
  \textit{Toxicity}    & $71,491\; (74.7\%)$ & $1,560\; (1.6\%)$  & $22,659\; (23.7\%)$ & $95,710$\\
  \textit{Hate Expert} & $6,155\; (67.6\%)$  & $953\; (10.5\%)$   & $2,002\; (22.98\%)$ & $9,110$\\
  \textit{Hate Speech} & $7,042\; (47.8\%)$  & $2,599\; (17.6\%)$ & $5,089\; (34.6\%)$  & $14,730$\\
  \textit{StormFront}  & $1,834\; (32.9\%)$  & $2,273\; (40.8\%)$ & $1,459\; (26.2\%)$  & $5,566$
\end{tabular}%
}
\centering
...
\begin{tabular}{lllll}
                       & Not Abuse Only      & Abuse Only         & Intersection        & Vocab size\\\hline
  \textit{Offence}     & $3,199\; (19.2\%)$  & $7,978\; (47.9\%)$ & $5,486\; (32.9\%)$  & $16,663$\\
  \textit{Toxicity}    & $71,493\; (74.7\%)$ & $1,560\; (1.6\%)$  & $22,659\; (23.4\%)$ & $95,712$\\
  \textit{Hate Expert} & $6,231\; (67.9\%)$  & $971\; (10.6\%)$   & $1,979\; (21.6\%)$  & $9,181$\\
  \textit{Hate Speech} & $7,074\; (47.7\%)$  & $2,653\; (17.9\%)$ & $5,107\; (34.4\%)$  & $14,834$\\
  \textit{StormFront}  & $1,804\; (32.7\%)$  & $2,240\; (40.7\%)$ & $1,466\; (26.6\%)$  & $5,510$
\end{tabular}%
}
...
Considering \cref{tab:LIWC_vocab_overlaps,tab:word_vocab_overlaps,tab:BPE_vocab_overlaps} that display the numbers of tokens that are unique to- \ZTedit{and shared by} each class \ZTedit{in each dataset}\ZTdelete{ and the numbers that are shared for each dataset}, there are some clear implications for my research question.
First, only minor distributional shifts \ZTedit{occur} between word-based vocabularies (see \cref{tab:word_vocab_overlaps}) and BPE-based vocabularies (see \cref{tab:BPE_vocab_overlaps}).
...
Second, observing the differences between LIWC vocabulary distributions (see \cref{tab:LIWC_vocab_overlaps}) and the word vocabulary distributions, it is clear that \ZTdelete{the distributional changes are large and that the LIWC-based representation has large ramifications on the datasets and subsequently on the models optimised for the task}\ZTedit{there are large distributional changes, which have large ramifications for the datasets and subsequently the models optimised on them}.
\ZTdelete{For instance,}\ZTedit{This means that} as the vast majority of tokens are shared between \ZTdelete{both}\ZTedit{the} classes, there are fewer potential signals for models to over-fit to.
\ZTedit{Disregarding token interactions, a word-based model optimised on the \textit{Toxicity} dataset may overfit to $72,929$ unique tokens ($76.3\%$ of all unique tokens in the dataset).
Similarly disregarding token interactions, a LIWC-based model is only provided with $136$ unique tokens ($13.3\%$ of all unique tokens in the dataset) to which it can overfit to.}
\ZTdelete{For instance, where a word-based model optimised on the \textit{Toxicity} dataset may have at least $72,979$ unique tokens across the classes, that is $76.3\%$ of all unique tokens in the dataset that the model can potentially identify and embody spurious correlations on.
...
To ensure that the baseline models use the most appropriate \ZTedit{parameters}, I perform a cross-validated grid-search (as implemented by \citet{Pedregosa:2011}) over the possible settings of the model parameters for each model.
...
For prediction on other datasets, all datasets are passed through vectoriser \ZTedit{that is} fitted to the optimisation data.
...
I use a single optimisation procedure for all models to \ZTdelete{control the influence of}\ZTedit{limit the} confounding factors in the optimisation process.
...
Within each epoch, batches of the data \ZTdelete{through} are passed through the model for prediction during optimisation.
\ZTedit{Following this prediction, the loss is back-propagated through the model, updating the internal representation in the process.}
\ZTdelete{The loss following the model performance is then back-propagated through model, updating the internal representation in the process.}
This process is repeated for the assigned maximum number of epochs, or \ZTedit{until} the model triggers the early stopping \citep{Prechelt:1998} criteria. 
\ZTedit{The early stopping criteria} is that the computed loss on the validation set has been strictly increasing for at least $15$ epochs.
...
The values for the learning rate are sampled from a uniform distribution while the batch size and number of epochs to optimise for are sampled from a \ZTedit{pre-defined categorical set}\ZTdelete{categorical distribution}.
More generally, the values for all hyper-parameters, asides from dropout and the learning rate are sampled from a \ZTedit{pre-defined} categorical \ZTedit{set}\ZTdelete{distribution}.
...
The LSTM consists of an input embedding layer, a uni-directional LSTM layer, a linear output layer, and a softmax to produce the class \ZTdelete{probabilities}\ZTedit{probability distribution}.
...
However as the CNN requires window sizes and number of filters to be set, these are added as hyper-parameters to tune. The hyper-parameters \ZTedit{that are used to tune this}\ZTdelete{tuned on the} model are thus:
...
\ZTdelete{To answer these questions}\ZTedit{To examine these behaviours across datasets and architectures}, I develop three different neural network types and optimise these on three different data representations for each of the five datasets introduced in \cref{sub:LIWC_datasets}, resulting in $45$ different model architectures optimised.
...
Moreover, these strong in-domain performances also provide a weak suggestion that neural network architectures may be \ZTedit{well} suited for the task, as most of the of the datasets that are under consideration are small datasets.
...
Two potential reasons for this trend are, 1) the two datasets are the largest in datasets so more general patterns may be learned, 2) as the notion of `offensive' as constructed by \citet{Davidson:2017} bears strong similarities with the notion of `toxicity' constructed by \citet{Wulczyn:2017}, \ZTedit{they may yield}\ZTdelete{yielding in} subsets of the datasets that strongly share similarities with each other.
...
In observing the \texttt{F1-scores} on the validation sets in \cref{fig:davidson_dev_f1,fig:wulczyn_dev_f1,fig:waseem_dev_f1,fig:waseem_hovy_dev_f1,fig:garcia_dev_f1}, a general pattern emerges in which most models, regardless of input type, display similar learning curves.
...
In addition to these two exceptions, the models that are optimised for the \textit{StormFront} dataset (see \cref{fig:garcia_dev_f1}) \ZTdelete{in which three}\ZTedit{display three} patterns\ZTdelete{emerge}:
...
\ZTdelete{As these models are being optimised, there is an early sharp increase in the model performances followed by reaching a plateau and only post minor improvements in the later epochs.}
...
Although the experiments I conduct with the dataset keep the number of documents lower than the total annotated set in order to maintain a balanced dataset, the dataset does in some cases \ZTedit{allow for models that out-perform models optimised on larger datasets, in terms of out-of-domain prediction.}
\ZTdelete{out-perform models that are optimised on larger dataset for out-of-domain prediction, i.e. in }
\ZTedit{Specifically, }the MLP models evaluated on the \textit{Offence} data (see \cref{fig:davidson_MLP_test}, where the StormFront LIWC-based model out-performs several other models that are optimised on larger datasets.
...
However here it is also clear that there is \ZTdelete{an influence of dataset size}\ZTedit{a relationship between dataset size and out-of-domain performance}.
\ZTedit{The models optimised on the larger dataset (i.e. \textit{Hate Speech} with $~16,000$ documents) have better performance on the evaluation set of the smaller (the \textit{Hate Expert} dataset with $~7,000$ documents) than the other way around.}
\ZTdelete{Where the \textit{Hate Speech} dataset consists of $~16,000$ documents, the \textit{Hate Expert} dataset consists of $~7,000$ documents, which is also apparent from the fact that the models optimised on the \textit{Hate Speech} dataset perform better on the \textit{Hate Expert} evaluation set than the models optimised on the \textit{Hate Expert} dataset perform on the \textit{Hate Speech} dataset.}
...
Where \citet{Garcia:2019} specifically seek out content from a white supremacist forum for their dataset, \citet{Waseem:2016,Waseem-Hovy:2016} sample from Twitter by searching for keywords that were likely to result in a large set of \ZTedit{of gendered and racialised} abuse.
...
As I can establish that LIWC-based data representation is appropriate for \ZTdelete{neural network models} neural network methods, I turn to ask what the influence of recurrence is on the performance of models when predicting on in-domain and out-of-domain datasets through the use of LSTM models.

On examining the test scores on the LSTM models (see \cref{fig:davidson_LSTM_test,fig:wulczyn_LSTM_test,fig:waseem_LSTM_test,fig:waseem_hovy_LSTM_test,fig:garcia_LSTM_test}), \ZTedit{I find that} the performance of MLPs and LSTMs in general are competitive with one another \ZTdelete{as}\ZTedit{and} it is dependent on the dataset, which \ZTdelete{model} architecture will work best.
...
\ZTedit{The LIWC-based models optimised on the \textit{Toxicity} dataset tend to out-perform other models optimised on the \textit{Toxicity} dataset, when evaluated on out-of-domain datasets.}
\ZTdelete{These models frequently out-perform the other models optimised on the \textit{Toxicity} dataset when evaluated on out-of-domain datasets.}
These results suggest that LIWC-based modelling may provide for improved out-of-domain performances when \ZTdelete{they}\ZTedit{the models} are optimised on large datasets or \ZTdelete{they} are being applied to data with which there are shared attributes in terms of annotation goal.
...
\ZTedit{However,} on the largest dataset, the \textit{Toxicity} dataset, the LIWC-based LSTM is slower to finish optimising than it's surface form counter parts.
...
In the medium sized datasets however, the relation between optimisation time and vocabulary minimisation \ZTedit{is} clear as the LIWC-based models tend to take less time to optimise than counter-part, as is apparent \ZTdelete{from some exceptions} in \cref{fig:davidson_train_time,fig:waseem_train_time,fig:waseem_hovy_train_time}.
...
Conversely, some LIWC-based models \ZTdelete{in some cases} see a lesser drop in performance on external data, if not \ZTedit{an outright increase in} \ZTdelete{increased} performances.
...
Further, the LIWC-based models, \ZTdelete{like}\ZTedit{similarly to} the Perspective API, do not take into consideration the context within which documents exist, as such they are similarly disembodied from the context they purport to model.
...
Thus, through a narrowing of the signals, that is tokens, \ZTdelete{which}\ZTedit{that} can constitute dirt, the boundaries which \ZTedit{are} subject to sanitisation become more porous along certain axes, providing solace for communities that are not recognised by the model.
At the same time, such algorithmic boundary-making is also made less porous to \ZTedit{the} threat of communities that are seen, and seen negatively, through the increased and incorrect sanitisation efforts of the model.
...
In the cases proposed by David Auerbach (see \autoref{fig:auerbach} and cases $15$-$40$ in \autoref{tab:LIWC-Perspective}), we see \ZTdelete{however how}\ZTedit{that} the LIWC-based model fails to capture many diverse forms of abuse, while somewhat surprisingly, capturing other forms.
...
Case $20$ and $32$ \ZTdelete{similarly} display surprising results, where the LIWC-based model correctly classifies both cases, \ZTdelete{while}\ZTedit{even though} case $20$ does hold a negation.
...
Such misclassification also pose inherent risk to any communities that are not recognised by the LIWC-based classifier while also offering space \ZTedit{for some communities} to exist.
The inability of both models to distinguishing signals from the signs that threaten the communities function as a double edged sword that will require additional content moderation strategies for such unrecognised communities\ZTdelete{in the case of the LIWC-based classifier}.
...
In particular, I find that out-of-domain model performances are contingent on similarities in the data sampling process \ZTdelete{or}\ZTedit{and} the goals of objectives of annotating data.
...
\ZTdelete{Addressing the second aim of this chapter, to}\ZTedit{Further, I} investigate the implication of using LIWC to represent documents on the computational, and thus environmental costs of developing machine learning models.
\ZTedit{I} show that optimisation time of neural network models has a relation to the size of the surface token vocabulary size and that models that make use of LIWC can provide competitive in-domain results and, in some instances out-perform on out-of-domain evaluation sets.
