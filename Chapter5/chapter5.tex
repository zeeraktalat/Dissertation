...
The use of n-grams as features provides a similar double-edged benefit, where models learn sequences of words \ZTedit{that may be very differently distributed in each class}.
...
\ZTedit{
  ...
  A large body of work is dedicated to the mathematical \ZTedit{and algorithmic} induction of what features to retain and which to omit \citep[e.g.]{May_2010,Witschel_2005}.
...
The dataset is initially split into a training and evaluation set, I \ZTedit{further} create a validation set by extracting a stratified sample from the training data, retaining the class balance from the balanced subset.
...
\ZTdelete{For instance, where a word-based model optimised on the \textit{Toxicity} dataset may have at least $72,979$ unique tokens across the classes, that is $76.3\%$ of all unique tokens in the dataset that the model can potentially learn spurious correlations on.
...
For prediction on other datasets, all datasets are passed through vectoriser \ZTedit{that is} fitted to the training data.
...
A notable difference between the optimisation of linear models and their neural network counterparts is that linear models are only provided with the dataset once for each cross-validation set and the order of the documents in the dataset is not randomised whereas the neural network models iterate multiple times over the training dataset where the order of the documents in the dataset is \ZTedit{randomly} shuffled between each iteration.
...
\ZTedit{The models trained on the larger dataset (i.e. \textit{Hate Speech} with $~16,000$ documents) have better performance on the evaluation set of the smaller (the \textit{Hate Expert} dataset with $~7,000$ documents) than the other way around.}
