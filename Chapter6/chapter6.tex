...
As each task may be related only in terms of the abstraction required to understand the meaning of a given text, creating mappings between different classes from different tasks, e.g. sarcastic and abusive, is a complex task that in some cases may not be possible, as one class may not conveniently fit others, e.g. mapping moral sentiments to abuse and vice versa.
...
Here I take the latter approach, developing a MTL model that considers each task simultaneously and distinctly from all other tasks (see \cref{sub:mtl} for more details on how MTL functions).
...
\ZTedit{Given that discriminatory speech may also be produced in moments of high emotionality, obtaining a representation within the model of whether a statement was made with a basis in emotion or fact may be a useful signal for identifying abuse.}
...
While the dataset does not hold enough samples of intersectional abuse to optimise machine learning models for this, I believe that this reflects the particular interests and biases of authors of datasets, rather than an inherent challenge.
Developing datasets for abuse can provide for significant challenges, such as dealing with the notion of ground truth and recruiting annotators that are attuned to abuse across intersections of identity \citep{Waseem:2016}, sampling \citep{Wiegand:2019}.
Neither of these issues are provide fundamental limits to the ability of optimising models for identifying intersectional abuse, however they betray the interests, subjectivities, and priorities of researchers within the field.
