...
Here, I address the questions of how speaker intentionality and mental state can be represented through the use of the Linguistic Inquiry and Word Count (LIWC) categories and examine what the influence of LIWC is on model generalisability.
By experimenting with three different neural network architectures: a Multi-Layered Perceptron (MLP), a Long-Short Term Memory (LSTM) network and a Convolutional Neural Network (CNN) and subjecting them to different representations of input data, I examine how each data representation influences model performances for each architecture and each input type.
The three data data representations that I choose to represent documents through are 1) the input word tokens, 2) sub-words as tokens, and 3) LIWC categories as tokens.
I choose the latter two representation forms because using sub-words can minimise the number of out-of-vocabulary tokens and LIWC because the LIWC software and dictionary are developed for the purposes of gaining insight into the mental and emotional state of speakers.
One impact of using the LIWC dictionary is that the possible size of the vocabularies in the datasets used shrink from several thousands of unique tokens to much smaller sets of approximately $1000$ unique tokens, resulting in a maximal decrease of $99.8\%$ in the vocabulary sizes.
Given this reduction in vocabulary size, I hypothesise that there will be some impacts on model training time and therefore examine the temporal impacts of pre-processing documents into their LIWC categories on neural network models.
Finally, I investigate the implications of using documents transformed into their LIWC categories as input to neural network models as the LIWC categories map to a much smaller vocabulary space.
